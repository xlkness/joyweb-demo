  ...
  <div>
    <el-scrollbar max-height="45vh">
      ...
      >
        ...
        >
          <el-input v-model="cmdField.value"></el-input>
        </el-form-item>
        <el-form-item>
          <el-row :gutter="20">
            <el-col :span="12">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // console.log(commandserver, command)

    for (let i=0; command.base_req_params.fields && i<command.base_req_params.fields.length; i++) {
      let curField = command.base_req_params.fields[i]
      const fieldData = {
        key: curField.name,
    ...
    const submitExecForm = (formEl: FormInstance | undefined) => {
      ...
      formEl.validate((valid, fields) => {
        ...
        if (valid) {
          ...
          let baseExecInfo = {
            name: command.base_req_params.name,
            fields: fieldsValue,
          }
          let execCommandInfo = {
            project: project,
      ...
      formEl.validate((valid, fields) => {
        ...
        if (valid) {
          ...
          likeCommand(execCommandInfo).then((res)=>{
            ...
            ElNotification({
              title: "收藏结果通知",
              message: "收藏指令[" + command.base_req_params.name + "]成功！",
              type: 'success',
              duration: 4000,
              "show-close": true,
            })
